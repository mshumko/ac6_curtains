...
with open(os.path.join(dirs.NORM_DIR, 'ac6_L_MLT_bins.csv')) as f:
    keys = next(f).rstrip().split(',')
    bins = {}
    for key in keys:
        bins[key] = next(f).rstrip().split(',')
        bins[key] = list(map(float, bins[key]))
with open(os.path.join(dirs.NORM_DIR, 'ac6_L_MLT_norm.csv')) as f:
    reader = csv.reader(f)
...
draw_L_contours(ax[0], 'k')
draw_L_contours(ax[1], 'k')
L_labels, L_labels_names = draw_L_contours(ax[2], 'k')
L_label_colors = ['k', 'k', 'k']

### PLOT TWEEKS ###
ax[0].set_title(f'(a) AC6 microburst distribution', y=1.08)
ax[1].set_title(f'(b) Normalized AC6 microburst distribution', y=1.08)
