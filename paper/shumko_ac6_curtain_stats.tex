...
AC6 does not have the necessary pitch angle, $\alpha$, resolution to differentiate between drifting and precipitating electrons and test the \citeA{Blake2016} hypothesis. Instead we use AC6's position and Earth's asymmetric magnetic field and the South Atlantic Anomely to observe particles that are either drifting or immediately lost in the drift loss cone, and only immediately lost in the bounce loss cone (BLC). These concepts are described below.
...
Without pitch angle resolution, we use AC6's location in LEO to isolate particles that are trapped, barely-trapped, and immediately lost. Above the SAA, AC6 will observe, but not differentiate, particles that are trapped, barely-trapped, and immediately precipitating. In the region outside of the SAA and it's conjugate point, AC6 will observe particles barely trapped in the drift loss cone and immediately precipitating in the bounce loss cone. Lastly, above the SAA conjugate point, AC6 only observes immediately precipitating particles. If a particle is observed by AC6, then its pitch angle must be in the loss cone. If the particle's local pitch angle is less than $90^\circ$ then it will travel to altitudes below AC6, and potentially be lost. On the other hand, if the particle's local pitch angle is near $90^\circ$ then it will mirror at or below AC6. This particle will then gyrate towards the SAA where the particle's mirror point is in the atmosphere or below sea level. The particle is thus immediately lost.

We estimated the bounce loss cone region in the North Atlantic Ocean using magnetic field modeling. We used the IRBEM-Lib magnetic field library \cite{irbem} to estimate the local magnetic field strength. Then we traced the magnetic field line that AC6 is on to the southern hemisphere and searched for the position along that field line with an equivalent field strength. If that position is 100 kilometers or less above Earth's surface we considered that the BLC. Furthermore we noted where the equivalent field strength is below sea level as a more stringent bounce loss cone criteria.
