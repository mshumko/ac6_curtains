...
We estimated the bounce loss cone region in the North Atlantic Ocean using magnetic field modeling. We used IRBEM-Lib \cite{irbem} to estimate the local magnetic field strength. Then we traced the magnetic field line that AC6 is on to the southern hemisphere and searched for the position along that field line with an equivalent field strength. If that position is 100 kilometers or less above Earth's surface we considered that the BLC. Furthermore we noted where the equivalent field strength is below sea level as a more stringent bounce loss cone criteria.
