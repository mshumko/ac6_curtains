...
\textcolor{red}{Talk about the DLC here? Or earlier?}

The concept of the bounce loss cone pitch angle is traditionally defined as the pitch angle of a particle that will mirror at 100 km where collisions with the atmosphere become more numerous and the particle is likely to be lost. This pitch angle, however small, is always defined but most LEO spacecraft do not have the pitch angle resolution to identify particles that will be immediately lost. A particle can also be pseudo-trapped if it has a pitch angle between the bounce loss cone and the drift loss cone. A particle with a pitch angle smaller than the drift loss cone and larger than the bounce loss cone will not immediately precipitate, rather it will drift around the Earth and precipitate in the SAA where the mirror point magnetic field strength is at altitudes below 100 km and the particle will be lost.  


 and locations in LEO where a particle will drift around the 
