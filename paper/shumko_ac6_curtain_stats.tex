...
The distribution of curtains in L and MLT is shown in Fig. \ref{l_mlt_dist}. Figure \ref{l_mlt_dist}a shows the distribution of the observed curtains, while Fig. \ref{l_mlt_dist}b shows the same distribution normalized by the number of quality 10 Hz samples (flag = 0 in the data files) that both AC6 spacecraft took at the same location in each L-MLT bin. In Fig. \ref{l_mlt_dist}a and \ref{l_mlt_dist}b, the bins where no curtains were observed are white. The AC6 sampling distribution is shown in Fig. \ref{l_mlt_dist}c, whose white bins show where AC6 did not take any 10 Hz data at the same location. The normalized curtain distribution in Fig. \ref{l_mlt_dist}b shows an enhanced curtain occurrence in the outer radiation belt ($ \mathrm{L} \approx 5-8$) with the largest peak in the pre-midnight MLT sector.
...
Some curtains with a $< 1$ second duration suggest that past microburst observations included curtains, so the microburst impact on the atmosphere and the radiation belt is overestimated. Curtains with durations up to 3 seconds, on the other hand, could be related to precipitation bands. However, precipitation bands typically have a tens of seconds duration so curtains and precipitation bands are unlikely to be related.
...
\subsection{When and Where Are Curtains Observed}
Figure \ref{l_mlt_dist}b shows that curtains likely originate in the outer radiation belt and are observed relatively more in the pre-midnight than late-morning MLT regions. Furthermore, curtains are more often observed at higher L shells near midnight MLT, however the sampling statistics at high L are limited because AC6 rapidly crosses high L shells. Nevertheless, Fig. \ref{l_mlt_dist}b hints that curtains near midnight MLT were observed at L shells possibly outside the outer radiation belt. Lastly, Figure \ref{ae_dist}b shows that curtains are associated with an enhanced AE. While it is not a direct comparison (due to the electron energy and the binning scheme), \citeA{Douma2019} showed that the number of microburst observed by SAMPEX increases with increasing AE, up to about $\mathrm{AE} = 300 \ \mathrm{nT}$.
