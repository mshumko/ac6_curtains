...
The handful of curtains observed in the bounce loss cone, and shown in Fig. \ref{fig3}, \change{put the  Blake and O'Brien (2016) curtain drift hypothesis into question.}{were not drifting because locally-mirroring electrons had a conjugate mirror point below sea level. Therefore, these curtains can not be explained by the Blake and O'Brien (2016) drifting microburst hypothesis.} One possible explanation is that the drifting curtain electrons were observed at the end of their drift orbit. However, because these examples were seen far from the western edge of the BLC, any drifting electrons would have been lost before AC6 observed them. The curtain precipitation persisted for multiple bounce periods ($\approx 1.5$ seconds for 35 keV electrons in this region), suggesting that the curtain generation mechanism must be capable of persistently scattering electrons. \add{This rules out chorus waves as the source of these curtains, and we will now consider the second hypothesized mechanism introduced in section 1.}

\subsection{An Alternative Curtain Generation Mechanism}
\remove{The scattering mechanism must be radially localized; if we assume the mechanism is at the magnetic equator, it must be on a scale of a few hundred kilometers.} One candidate mechanism, \add{that can generate curtains,} is a direct current electric field that is parallel to the background magnetic field, that lowers the electron mirror point to AC6 altitudes. To find the minimum potential we assume the electron is barely trapped and has a 100-kilometer conjugate mirror point altitude in the SAA, so initially the electron will mirror above AC6 in the bounce loss cone region. 
...
The range of estimated potentials is typical for the inverted-V discrete aurora. \citeA{Partamies2008}, using observations made by the Fast Auroral SnapshoT (FAST) mission, reported that auroral inverted-V electron precipitation structures, with electron energies up to a few tens of keV, were accelerated by 2-4 kV parallel potentials. The inverted-V structure and curtains share several similarities including: latitudinal width, high occurrence rate in the midnight MLT region, and the maximum inverted-V energy extends into tens of keV \cite<e.g.>{Marklund2011, Thieman1985}. AC6's dos1, with its 35 keV electron threshold, may by observing the highest energy tip of the inverted-V aurora. A possible connection between the inverted-V structures is intriguing, but by itself AC6 cannot easily test this hypothesis. To investigate further, a follow-on study could look at ground-based auroral imager data and look for meso-scale auroral arcs when AC6 observed curtains overhead.
...
\item Some curtains continuously precipitate into the atmosphere for multiple seconds, \add{limited by AC6's short in-track lag.}
\end{enumerate}
