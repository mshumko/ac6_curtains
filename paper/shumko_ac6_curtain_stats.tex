...
\textcolor{red}{What does Lauren mean by too general... many diff types? } The range of estimated potentials is typical for the \textcolor{red}{inverted-V (discrete)} aurora. \citeA{Partamies2008} used the observations made by the Fast Auroral SnapshoT (FAST) mission. They reported that the auroral inverted-V electron precipitation structures, with electron energies up to a few tens of keV, were accelerated by 2-4 kV parallel potentials. The inverted-V structure and curtains share several similarities including: \textcolor{red}{the maximum inverted-V energy is not too far off 30 keV, and perhaps the same given the cross-calibration between instruments}, latitudinal width, and high occurrence rate in the midnight MLT region. A possible connection between the inverted-V structures is intriguing, but by itself AC6 cannot easily test this hypothesis. To investigate further, a follow-on study could look at ground-based auroral imager data and look for meso-scale auroral arcs when AC6 observed curtains overhead.

\textcolor{red}{Make this paragraph less tacked on. To elaborate on this, the quantification of this will be looked at in the future.}
Outside of the BLC, the lack of pitch angle and energy information makes the AC6 electron data ambiguous. Nevertheless, the curtains observed in the BLC suggest that at least some curtains locally precipitate for multiple seconds. \textcolor{red}{Even if these curtains are the exception and other curtains are drifting, the drifting curtains will still precipitate within one drift period. Therefore, the impact of energetic curtain precipitation into the atmosphere needs to be considered.} Energetic electron precipitation produces odd Nitrogen ($\mathrm{NO_X}$) that is currently underestimated by atmospheric models such as the widely-used Whole Atmosphere Community Climate Model (WACCM) \cite<e.g.>{Randall2015}. \textcolor{red}{Curtain precipitation could explain the lack of atmospheric odd Nitrogen.} A comprehensive study of the curtain impact on the atmosphere can be done with an AC6-like mission with pitch angle and energy resolution. 
